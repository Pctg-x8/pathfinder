...
impl<FK> FontContext<FK> where FK: Clone + Hash + Eq + Ord {
    ...
    pub fn load_glyph_indices_for_characters(&mut self, font_instance: &FontInstance<FK>, characters: &[u16])
        ...
        let result =
            core_text_font.get_glyphs_for_characters(characters.as_ptr(), glyphs.as_mut_ptr(), characters.len() as _);
    ...
    pub fn ascent(&mut self, font_instance: &FontInstance<FK>) -> Result<f64, ()> {
        self.ensure_core_text_font(font_instance).map(|f| f.ascent())
    }
